...
class HighlightsShortcode extends ViewableData
{
    private static $shortcode = 'highlights';
    private static $singular_name = 'Highlights';
    private static $plural_name = 'Highlights';
    ...
    public static function parse_shortcode($arguments, $content, $parser, $shortcode)
    {
        $data = new ArrayData(array(
            'Content' => $content,
            'Color' => (array_key_exists('color', $arguments)) ? $arguments['color'] : false,
        ));
        if (str_contains($_SERVER["QUERY_STRING"], 'fj=1') || str_starts_with($_SERVER['REQUEST_URI'], '/for-journalists')) {
            return $content;
        }
        // render with template
        return $data->renderWith('Northwestern\Now\Shortcodes\HighlightsShortcode');
    }
    ...
    public function getShortcodeFields()
    {
        ...
        $fields = FieldList::create(
            LabelField::create('intro', 'Select highlights text before creating shortcode or enter it between [highlights] tags after insertion.'),
            OptionsetField::create('color', 'Color', [
                'purple' => 'Purple',
                'grey' => 'Grey',
            ], 'purple'
            )
