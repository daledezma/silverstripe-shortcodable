...
class StatisticsShortcode extends ViewableData
{
    private static $shortcode = 'statistic';
    private static $singular_name = 'Statistics';
    ...
    {
        if (!array_key_exists('number', $arguments) || !$arguments['number']) {
            return 'Statistics: Number is required';
        }
        ...
        $data = new ArrayData(array(
            'Content' => $content,
            'Number' => (array_key_exists('number', $arguments)) ? $arguments['number'] : false,
            'Label' => (array_key_exists('label', $arguments)) ? $arguments['label'] : false,
        ));

        // render with template
        return $data->renderWith('Northwestern\Now\Shortcodes\StatisticsShortcode');
    }
